...
### 關於 Hash 
    ## Hash是甚麼(What)?
        ...
        * A data structure that stores key-value pairs by using a hash function to compute an index into an array of buckets.
    ## Hash甚麼情況會用到(When)?
...
### Time Complexity
    ## Chaining(鏈結法): ※ $\alpha = \frac{N}{M}$
        Insertion: `Best Case`: $O(1)$ `Average Case`: $O(1+\alpha)$ `Worst Case`: $O(N)$
        Search:    `Best Case`: $O(1)$ `Average Case`: $O(1+\alpha)$ `Worst Case`: $O(N)$
        Deletion:  `Best Case`: $O(1)$ `Average Case`: $O(1+\alpha)$ `Worst Case`: $O(N)$
    ## Open Addressing: ※ $\alpha = \frac{N}{M}$
...
### 運作流程(Operation):
    ...
    ## Hash function:
        ...
        * Implementation:
            ...
            2. Multiplication method: $h(k) = \lfloor m (k A \bmod 1) \rfloor$, 其中 $0 < A < 1$，通常取 $A \approx 0.618$ (黃金比例的小數部分)
            3. Folding method: 
                * spilt key into several parts and sum them up
                * if $k = 123456$ then maybe $h(k) = (123 + 456) \bmod m$
            4. String Hashing:
        ...
        * Solution:
            1. Chaining(鏈結法):
                ...
                * Cons:
                    ...
                    2. dynamic size: 透過不斷的`realloc`/`malloc`，除了會不斷地對記憶體進行操作，實際上也會比array的速度來的慢(directly access)
                ...
                * 種類:
                    ...
                    2. quadratic probing:
                        ...
                        * Cons:
                            ...
                            (when different keys that hash to the same initial index $h(k)$)
                    3. double hashing:
...
### Comparison:
    ## Static Hashing:
        * Size : `Fixed`
        * Load Factor : $\alpha = \frac{N}{M}$ higher -> more collision
        * Hash Function : `Constant` 
        * Rehashing : `Entire table must be rebuilt`
        * Implement: `Simple`
